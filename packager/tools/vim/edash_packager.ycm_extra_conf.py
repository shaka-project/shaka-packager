...
# USAGE:
#
#   1. Point to this config file in your .vimrc:
...
# Usage notes:
#
#   * You must use ninja & clang to build.
#
#   * You must have run gyp_packager and build the Debug version.
#
#
# Hacking notes:
#
...
# Slightly modified the Chromium version so that it works with the packager.
...
def FindChromeSrcFromFilename(filename):
  ...
  while not (PathExists(curdir, 'DEPS')
             and PathExists(curdir, 'packager')
...
def GetClangCommandFromNinjaForFilename(chrome_root, filename):
  ...
  # TODO(rkuroiwa): The lines above can be uncommented and the line below can be
  # deleted when we pull tools/vim/ninja_output.py. For now, just look at the
  # Debug output directory.
  out_dir = os.path.realpath(os.path.join(chrome_root, 'out', 'Debug'))
