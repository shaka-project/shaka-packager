...
TEST(H265VideoSliceHeaderParserTest, BasicSupport) {
  const uint8_t kExtraData[] = {
    // Header
    0x01, 0x01, 0x40, 0x00, 0x00, 0x00, 0x90, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x96, 0xF0, 0x00, 0xFC, 0xFD, 0xF8, 0xF8, 0x00,
    0x00, 0x0F, 0x03,
    // VPS
    0x20, 0x00, 0x01,
    0x00, 0x17,  // Size = 23
    0x40, 0x01, 0x0C, 0x01, 0xFF, 0xFF, 0x01, 0x40, 0x00, 0x00,
    0x03, 0x00, 0x90, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00,
    0x96, 0xBC, 0x09,
    // SPS
    0x21, 0x00, 0x01,
    0x00, 0x2D,  // Size = 45
    0x42, 0x01, 0x01, 0x01, 0x40, 0x00, 0x00, 0x03, 0x00, 0x90,
    0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x96, 0xA0, 0x03,
    0xC0, 0x80, 0x11, 0x07, 0xCB, 0x96, 0xF4, 0xA4, 0x21, 0x19,
    0x3F, 0x8C, 0x04, 0x04, 0x00, 0x00, 0x03, 0x00, 0x04, 0x00,
    0x00, 0x03, 0x01, 0x68, 0x20,
    // PPS
    0x22, 0x00, 0x01,
    0x00, 0x08,  // Size = 8
...
TEST(H265VideoSliceHeaderParserTest, BasicStereoVideoSupport) {
  const uint8_t kExtraDataL0[] = {
    // Header
    0x01, 0x21, 0x60, 0x00, 0x00, 0x00, 0xB0, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x78, 0xF0, 0x00, 0xFC, 0xFD, 0xF8, 0xF8, 0x00,
    0x00, 0x03, 0x04,
    // VPS
    0xA0, 0x00, 0x01,
    0x00, 0x37,  // Size = 55
    0x40, 0x01, 0x0C, 0x11, 0xFF, 0xFF, 0x21, 0x60, 0x00, 0x00,
    0x03, 0x00, 0xB0, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00,
    0x78, 0x93, 0x05, 0x6F, 0x78, 0x20, 0x00, 0x28, 0x24, 0x59,
    0x72, 0x60, 0x20, 0x00, 0x00, 0x03, 0x00, 0xF8, 0x80, 0x00,
    0x00, 0x03, 0x00, 0x07, 0x88, 0xD0, 0x78, 0x00, 0x44, 0x0A,
    0x01, 0xE5, 0xC9, 0x7D, 0x20,
    // SPS
    0xA1, 0x00, 0x01,
    0x00, 0x28,  // Size = 40
    ...
    0xA2, 0x00, 0x01,
    0x00, 0x0D,  // Size = 13
    0x44, 0x01, 0xC5, 0xE3, 0x0F, 0x09, 0xC1, 0x80, 0xC7, 0xB0,
    0x9A, 0x01, 0x40,
    // SEI
    0xA7, 0x00, 0x01,
    0x00, 0x09,  // Size = 9
  ...
  const uint8_t kExtraDataL1[] = {
    // Header
    0x01, 0xF0, 0x00, 0xFC, 0xCB, 0x02,
    // SPS
    0xA1, 0x00, 0x01,
    0x00, 0x0E,  // Size = 14
    0x42, 0x09, 0x0E, 0x85, 0xB9, 0x32, 0x6B, 0xBE, 0x80, 0x2E,
    0xD0, 0xA1, 0x34, 0x04,
    // PPS
    0xA2, 0x00, 0x01,
    0x00, 0x0E,  // Size = 14
    0x44, 0x09, 0x48, 0x5A, 0x43, 0x0F, 0x09, 0xC1, 0x80, 0xC7,
    0xB4, 0x9A, 0x01, 0x40
  };
  ...
  const uint8_t kDataL1Intra[] = {
    // Incomplete data, but we only care about the header size.
    0x26, 0x09, 0x90, 0x80, 0x3C, 0xC6, 0xAC, 0x30, 0x01, 0x20,
    0x6A, 0x01, 0x37, 0x28, 0x7F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xF8,
    // Below are just arbitrary data after the slice header.
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
  };
  const uint8_t kDataL0Inter[] = {
    // Incomplete data, but we only care about the header size.
    0x02, 0x01, 0xC4, 0x03, 0x7C, 0xC4, 0xAC, 0x30, 0x00, 0x0F,
    0xAA, 0x00, 0x1B, 0x30, 0x7F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xF8,
    // Below are just arbitrary data after the slice header.
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
  };
  const uint8_t kDataL1Inter[] = {
    // Incomplete data, but we only care about the header size.
    0x02, 0x09, 0xA1, 0x00, 0x9A, 0x00, 0xBD, 0x65, 0x89, 0x58,
    0x60, 0x00, 0x30, 0x2C, 0x00, 0x41, 0xC0, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xF0,
    // Below are just arbitrary data after the slice header.
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
  };
  ...
  const std::vector<uint8_t>
      extra_data_layer_0(kExtraDataL0, kExtraDataL0 + std::size(kExtraDataL0));
  H265VideoSliceHeaderParser parser;
  ASSERT_TRUE(parser.Initialize(extra_data_layer_0));
  const std::vector<uint8_t>
      extra_data_layer_1(kExtraDataL1, kExtraDataL1 + std::size(kExtraDataL1));
  ASSERT_TRUE(parser.InitializeLayered(extra_data_layer_1));

  Nalu nalu;
  ASSERT_TRUE(nalu.Initialize(Nalu::kH265, kDataL0Intra, std::size(kDataL0Intra)));
  EXPECT_EQ(17, parser.GetHeaderSize(nalu));
  ASSERT_TRUE(nalu.Initialize(Nalu::kH265, kDataL1Intra, std::size(kDataL1Intra)));
  EXPECT_EQ(19, parser.GetHeaderSize(nalu));
  ASSERT_TRUE(nalu.Initialize(Nalu::kH265, kDataL0Inter, std::size(kDataL0Inter)));
  EXPECT_EQ(19, parser.GetHeaderSize(nalu));
  ASSERT_TRUE(nalu.Initialize(Nalu::kH265, kDataL1Inter, std::size(kDataL1Inter)));
