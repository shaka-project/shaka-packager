...
bool AACAudioSpecificConfig::ConvertToADTS(std::vector<uint8_t>& audio_frame, const uint8_t* data, size_t data_size) const {
  ...
  audio_frame.insert(audio_frame.end(), kADTSHeaderSize, 0);

  audio_frame[0] = 0xff;
  audio_frame[1] = 0xf1;
  audio_frame[2] = ((audio_object_type_ - 1) << 6) + (frequency_index_ << 2) + (channel_config_ >> 2);
  audio_frame[3] = ((channel_config_ & 0x3) << 6) + static_cast<uint8_t>(size >> 11);
