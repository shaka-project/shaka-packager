...
TEST(LHEVCDecoderConfigurationRecordTest, Success) {
  // clang-format off
  const uint8_t kHevcDecoderConfigurationData[] = {
      0x01,  // version
      0x01,  // profile_indication
      ...
      0x0B,  // length_size_minus_one - just the 2 LSBs
      0x04,  // num_of_arrays
        // array 1
        0xA0,  // nal type = VPS
        0x00, 0x01,  // num nalus
          0x00, 0x38,  // nal unit length = 56
          0x40, 0x01, 0x0C, 0x11, 0xFF, 0xFF, 0x01, 0x60, 0x00, 0x00,
          0x03, 0x00, 0xB0, 0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00,
          0x78, 0x15, 0xC1, 0x5B, 0x00, 0x20, 0x00, 0x28, 0x24, 0xC1,
          0x97, 0x06, 0x02, 0x00, 0x00, 0x03, 0x00, 0xBF, 0x80, 0x00,
          0x00, 0x03, 0x00, 0x00, 0x78, 0x8D, 0x07, 0x80, 0x04, 0x40,
          0xA0, 0x1E, 0x5C, 0x52, 0xBF, 0x48,
        // array 2
        0xA1,  // nal type = SPS
        0x00, 0x01,  // num nalus
          0x00, 0x24,  // nal unit length = 36
          0x42, 0x01, 0x01, 0x01, 0x60, 0x00, 0x00, 0x03, 0x00, 0xB0,
          0x00, 0x00, 0x03, 0x00, 0x00, 0x03, 0x00, 0x78, 0xA0, 0x03,
          0xC0, 0x80, 0x11, 0x07, 0xCB, 0x88, 0x15, 0xEE, 0x45, 0x95,
          0x4D, 0x40, 0x40, 0x40, 0x40, 0x20,
        // array 3
        0xA2,  // nal type = PPS
        0x00, 0x01,  // num nalus
          0x00, 0x07,  // nal unit length = 7
          0x44, 0x01, 0xC0, 0x2C, 0xBC, 0x14, 0xC9,
        // array 4
        0xA7,  // nal type = SEI
        0x00, 0x01,  // num nalus
          0x00, 0x09,  // nal unit length = 9
          0x4E, 0x01, 0xB0, 0x04, 0x04, 0x0A, 0x80, 0x20, 0x80
  };

  const uint8_t kLhevcDecoderConfigurationData[] = {
      0x01,  // version
      0xF0, 0x00, 0xFC,
      0xCB,  // length_size_minus_one - just the 2 LSBs
      0x02,  // num_of_arrays
        // array 1
        0xA1,  // nal type = SPS
        // Number of NAL units
        0x00, 0x01,  // num nalus
          0x00, 0x0A,  // nal unit length = 10
          0x42, 0x09, 0x0E, 0x82, 0x2E, 0x45, 0x8A, 0xA0, 0x05, 0x01,
        // array 2
        0xA2,  // nal type = PPS
        0x00, 0x01,  // num nalus
          0x00, 0x09,  // nal unit length = 9
          0x44, 0x09, 0x48, 0x02, 0xCB, 0xC1, 0x4D, 0xA8, 0x05,
  };
  // clang-format on
  ...
  ASSERT_TRUE(hevc_config.Parse(kHevcDecoderConfigurationData,
                                std::size(kHevcDecoderConfigurationData)));
  EXPECT_EQ(4u, hevc_config.nalu_length_size());
  EXPECT_EQ("hev1.1.6.L120.b0", hevc_config.GetCodecString(FOURCC_hev1));
  EXPECT_EQ("hvc1.1.6.L120.b0", hevc_config.GetCodecString(FOURCC_hvc1));
  EXPECT_EQ(4u, hevc_config.nalu_count());
  ...
  EXPECT_EQ(2u, hevc_config.nalu_count());
  EXPECT_EQ(0x08, hevc_config.nalu(0).payload_size());
  EXPECT_EQ(0x42, hevc_config.nalu(0).data()[0]);
}
