...
TEST(H264ByteToUnitStreamConverter, NaluConversionWithSpsExtension) {
  ...
  const uint8_t kExpectedUnitStream[] = {0x00, 0x00, 0x00, 0x0A, 0x06,
                                         0xFD, 0x78, 0xA4, 0xC3, 0x82,
                                         0x62, 0x11, 0x29, 0x77};
  EXPECT_EQ(std::vector<uint8_t>(std::begin(kExpectedUnitStream),
                                 std::end(kExpectedUnitStream)),
            unit_stream);
  ...
  const uint8_t kExpectedDecoderConfig[] = {
      ...
      0x96, 0x01, 0x00, 0x0A, 0x68, 0xFE, 0xFD, 0xFC, 0xFB, 0x11, 0x12, 0x13,
      0x14, 0x15, 0xFD, 0xF8, 0xF8, 0x01, 0x6D, 0x33, 0x01, 0x57, 0x78};
  EXPECT_EQ(std::vector<uint8_t>(std::begin(kExpectedDecoderConfig),
                                 std::end(kExpectedDecoderConfig)),
            decoder_config);
}
