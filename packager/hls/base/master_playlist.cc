...
void BuildMediaTag(const MediaPlaylist& playlist,
  ...
  if (playlist.stream_type() == kAudio) {
    // According to HLS spec: https://tools.ietf.org/html/
    // draft-pantos-hls-rfc8216bis 4.4.6.1. CHANNELS is a quoted-string that
    // specifies an ordered, slash-separated ("/") list of parameters.
    // The first parameter is a count of audio channels, and the second
    // parameter identifies the encoding of object-based audio used by the
    // Rendition. HLS Authoring Specification for Apple Devices Appendices
    // documents how to handle Dolby Digital Plus JOC content.
