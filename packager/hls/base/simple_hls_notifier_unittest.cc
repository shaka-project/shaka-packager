...
TEST_F(SimpleHlsNotifierTest, LocalTargetDuration) {
  ...
  // that per_playlist_target_duration results in different target durations per playlist
  ...
  EXPECT_TRUE(notifier.NotifyNewSegment(stream_id1, segment_name1_1, kStartTime1_1,
                                        kDuration1_1, 203, kSize1_1));
  EXPECT_TRUE(notifier.NotifyNewSegment(stream_id1, segment_name1_2, kStartTime1_2,
                                        kDuration1_2, 203, kSize1_2));
  EXPECT_TRUE(notifier.NotifyNewSegment(stream_id2, segment_name2_1, kStartTime2_1,
                                        kDuration2_1, 203, kSize2_1));
  EXPECT_TRUE(notifier.NotifyNewSegment(stream_id2, segment_name2_2, kStartTime2_2,
                                        kDuration2_2, 203, kSize2_2));
  ...
  // When per_playlist_target_duration is enabled, each playlist should get its own
  // target duration based on its longest segment, not the global maximum.
  // Playlist1 should get 5 (based on its 5s segment)
  // Playlist2 should get 10 (based on its 10s segment)
  EXPECT_CALL(*mock_media_playlist1, GetLongestSegmentDuration())
      .WillRepeatedly(Return(kLongestSegmentDuration1));
  EXPECT_CALL(*mock_media_playlist1, SetTargetDuration(kExpectedTargetDuration1))
      .Times(1);
  EXPECT_CALL(*mock_media_playlist2, GetLongestSegmentDuration())
      .WillRepeatedly(Return(kLongestSegmentDuration2));
  EXPECT_CALL(*mock_media_playlist2, SetTargetDuration(kExpectedTargetDuration2))
      .Times(1);
